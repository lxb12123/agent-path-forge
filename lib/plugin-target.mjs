...
export function pluginMeta(targetDir) {
  ...
  return {
    name,
    description: p.description || `${name} — an agent plugin forged by Agent Plugin Kit.`,
...
export function pack(targetDir) {
  if (!hasGene(targetDir)) {
    throw new Error('not an Agent Plugin Kit project (run `inherit` first to grow at least the foundation)');
  }
