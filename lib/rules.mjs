...
const CLAUDE_START = '<!-- agent-plugin-kit:rules:start -->';
const CLAUDE_END = '<!-- agent-plugin-kit:rules:end -->';
...
// Write .cursor/rules/<name>.mdc + stamp the rules into the agent-plugin-kit managed block in CLAUDE.md.
export function compileRules(targetDir) {
  ...
  const body = `# Project rules (agent-plugin-kit)\n\n${claudeRulesBody(rules)}`;
